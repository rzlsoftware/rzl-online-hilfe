...
**Status:** ✅ Done — now uses **native Pagefind filtering**. The previously noted open item (program id derived client-side from the URL path instead of real Pagefind facets) is resolved: `src/components/MarkdownContent.astro` emits `data-pagefind-filter="program:<id>"` from each page's `program` frontmatter into the indexed body, and `programFilter.ts` mirrors the dropdown onto Pagefind's own (CSS-hidden) filter checkboxes instead of hiding rendered result nodes.

That replaced the earlier DOM-hiding approach (`9bea5b5`), which was fundamentally limited because Pagefind renders only 5 results at a time: hiding nodes could hide every loaded result while real matches sat further down the unfiltered list, the result count ignored the filter, and "load more" paged through results the filter had already removed. Concretely, searching `uva` with the Board filter reported "32 results" with nothing visible, while the one genuine Board match was never reached.

Also fixed alongside it:

- **German search UI.** Pagefind ships German strings, but its automatic language detection is dead code in the bundled build — `automatic_translations` is initialised to the English table and the `onMount` block that would pick a table from `<html lang>` (plus its `bcp-47` dependency) is tree-shaken away, so `<html lang="de">` had no effect. Supplying the strings explicitly via Starlight's i18n collection (`src/content/i18n/de.json`) makes them *overrides*, which take precedence and work regardless of that upstream bug.
- **Result counts / "load more".** Both are now produced by Pagefind against the filtered result set, so the count matches the filter and the button only appears when more matching results genuinely exist.
- **Where the results are.** Option labels carry per-program hit counts for the current query (e.g. `Alle Programme (32)`, `Board (1)`), and when the selected program has no hits but others do, an explanatory line is shown.
- **Search hit highlighting.** Following a result highlights the matched terms on the target page. `searchResultLinks.ts` appends the terms to the result links as repeated `?highlight=` params, and `highlightMatches.ts` marks them using Pagefind's own `pagefind-highlight.js` (mark.js). Pagefind's built-in `highlightParam` option was not usable: Starlight's `pagefind` Zod schema strips unknown keys, and setting it at runtime is racy because `pagefind.js` replaces the whole stored options object until the first search creates the instance. Rewriting the `href` is deterministic and also covers middle-click, "open in new tab" and "copy link address". The highlighter is only fetched when a `highlight` param is present, so normal navigation is unaffected.

  Highlighting is suppressed where it would only be noise: German stop words and single characters are dropped (`searchTerms.ts` — a stop-word list rather than a minimum length, so domain terms like `KV`/`BV`/`L19` survive), and nothing is marked at all when every term already appears in the heading that was navigated to, since searching a page or section by its name means there is nothing left to locate. The `<h1>` and the anchored heading are excluded from marking for the same reason. Highlights can always be dismissed with the on-screen control or Escape, which unwraps the marks and drops the param from the URL so a reload stays clean.
